---
title: "Vignette o2geosocial"
output:
  rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Vignette o2geosocial}
...
Finally, the distance matrix between regions is set up from the data table `dt_regions`, element of `toy_outbreak_short`. We use the column `population` to set up the population vector `pop_vect`. We compute the distance between each region into the distance matrix `dist_mat` from the R package [*geosphere*](https://cran.r-project.org/web/packages/geosphere/geosphere.pdf). 
...
In this section, we want to generate two maps to observe the geographical distribution of the importations, and the average number of secondary cases per region in `out1` and `out2`. The maps will be generated using the package [*ggplot2*](https://cran.r-project.org/web/packages/ggplot2/)

First, we download the shapefiles associated to the census tract in Ohio. They will be used to generate the background of the maps, and are available on the website [*census.gov*](https://www2.census.gov). We use the function `download.file()` to import the shapefile into the R session. If, when running `download.file()`, R cannot access the zip file, download the file following the URL link https://www2.census.gov/geo/tiger/GENZ2018/shp/cb_2018_39_tract_500k.zip and create a variable `temp` pointing at the zip file (e.g. `temp <- ~/Downloads/cb_2018_39_tract_500k.zip`). 

```{r download_zip, warning = FALSE, fig.width = 6.5, fig.height = 2.5}
library(ggplot2)
# Download the ZIP archive in a temporary folder
temp <- tempfile()
download.file(
  "https://www2.census.gov/geo/tiger/GENZ2018/shp/cb_2018_39_tract_500k.zip",
  temp)
```

We then extract the shapefile from the zip archive, read it using the package [*sf*](https://cran.r-project.org/web/packages/sf/index.html) and create one background map for each model.

```{r unzip_shp, warning = FALSE, fig.width = 6.5, fig.height = 2.5}
# Extract the shapefile into a temporary folder
unzip(temp, exdir = tempdir())
# Read the shapefile and create one map for each model
map1 <- sf::st_read(paste0(tempdir(), "/cb_2018_39_tract_500k.shp"),
                    quiet = T)
map2 <- map1
map1$model <- "Model 1"
map2$model <- "Model 2"
...
First, we create the intial distance matrix in the Stouffer's rank model `dist_mat_stouffer`:
...
We now write the function `cpp_stouffer_move_a` to replace the default movement function `cpp_move_a`. As the Stouffer's rank and the power law gravity models are close, we do not need to re-write `cpp_log_like`, the default function to compute the probability matrix. For other distance models, it may be required to create a new version of `cpp_log_like` and call it in the movement function. In `cpp_stouffer_move_a`, there is no parameter $b$, both spatial parameters are equal to $a$. We use the package [*Rcpp*](https://cran.r-project.org/web/packages/Rcpp/index.html) to source the new movement function.
